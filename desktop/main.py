...
def update_attendance(uid, current_time, status):
    ...
    if student:
        ...
        if status == 'ENTRY':
            ...
            if entry_time > config.LATE_CUTOFF:
                cursor.execute(
...
def start_system():
    ...
    if not current_time or not status:
        ...
        messagebox.showwarning("Input Error", "Please set both time and status.")
        return
    ...
    while True:
        try:
            if ser.in_waiting:
                ...
                handle_database_operations(uid, current_time, status)
        except serial.SerialException:
            display_message("Serial connection error.")
            break
...
def show_main_gui(current_time, status):
    ...
    threading.Thread(target=run_serial_communication, args=(ser, current_time, status), daemon=True).start()
